...
if __name__ == '__main__':
    ...
    # importing monthly adjusted closing price of DJI index for training data
    dow_jones_monthly_training = pd.read_csv('DJI_monthly_training.csv')
    market_returns_monthly_training = dow_jones_monthly_training.copy()
    market_returns_monthly_training["Market Returns"] = market_returns_monthly_training['^DJI'].pct_change()
    market_returns_monthly_training = market_returns_monthly_training.drop(columns = '^DJI').dropna()

    # importing monthly adjusted closing price of constituent stocks in DJI index for training data
    stock_data_monthly_training = pd.read_csv('dow_stock_monthly_training.csv', index_col=0)
    stock_data_monthly_training = stock_data_monthly_training.set_index('Date').astype(float).round(2)

    # importing quarterly adjusted closing price of DJI index for training data
    dow_jones_quarterly_training = pd.read_csv('DJI_trimonthly_training.csv')
    market_returns_quarterly_training = dow_jones_quarterly_training.copy()
    market_returns_quarterly_training["Market Returns"] = market_returns_quarterly_training['^DJI'].pct_change()
    market_returns_quarterly_training = market_returns_quarterly_training.drop(columns = '^DJI').dropna()

    # importing quarterly adjusted closing price of constituent stocks in DJI index for training data
    stock_data_quarterly_training = pd.read_csv('dow_stock_trimonthly_training.csv', index_col=0)
    stock_data_quarterly_training = stock_data_quarterly_training.set_index('Date').astype(float).round(2)

    # importing monthly adjusted closing price of DJI index for testing data
    dow_jones_monthly_testing = pd.read_csv('DJI_monthly_testing.csv')
    market_returns_monthly_testing = dow_jones_monthly_testing.copy()
    market_returns_monthly_testing["Market Returns"] = market_returns_monthly_testing['^DJI'].pct_change()
    market_returns_monthly_testing = market_returns_monthly_testing.drop(columns = '^DJI').dropna()
    
    # importing monthly adjusted closing price of constituent stocks in DJI index for testing data
    stock_data_monthly_testing = pd.read_csv('dow_stock_monthly_testing.csv', index_col=0)
    stock_data_monthly_testing = stock_data_monthly_testing.set_index('Date').astype(float).round(2)
    
    # importing quarterly adjusted closing price of DJI index for testing data
    dow_jones_quarterly_testing = pd.read_csv('DJI_trimonthly_testing.csv')
    market_returns_quarterly_testing = dow_jones_quarterly_testing.copy()
    market_returns_quarterly_testing["Market Returns"] = market_returns_quarterly_testing['^DJI'].pct_change()
    market_returns_quarterly_testing = market_returns_quarterly_testing.drop(columns = '^DJI').dropna()
    
    # importing quarterly adjusted closing price of constituent stocks in DJI index for testing data
    stock_data_quarterly_testing = pd.read_csv('dow_stock_trimonthly_testing.csv', index_col=0)
    stock_data_quarterly_testing = stock_data_quarterly_testing.set_index('Date').astype(float).round(2)
    
    # finding optimal (N,R) combinations for various KPIs for monthly portfolio rebalancing
    portfolio_selection_monthly = PortfolioSelection(stock_data_monthly_training,market_returns_monthly_training,"1mo")
    ...
    portfolio_selection_quarterly = PortfolioSelection(stock_data_quarterly_training,market_returns_quarterly_training,"3mo")
    ...
    portfolio_monthly = PortfolioConstruction(stock_data_monthly_testing)
    ...
    portfolio_monthly.returns_histogram(market_returns_monthly_testing)

    # plotting portfolio returns against market returns
    portfolio_monthly.market_portfolio_plot(market_returns_monthly_testing,"Monthly Returns, Combinations: (9, 3)")

    # calculating KPIs for 9,3 monthly rebalancing
    portfolio_monthly_KPI = KPI(portfolio_returns_monthly,market_returns_monthly_testing,"1mo")
    ...
    portfolio_quarterly = PortfolioConstruction(stock_data_quarterly_testing)
    ...
    # plotting portfolio returns against market returns
    portfolio_quarterly.market_portfolio_plot(market_returns_quarterly_testing, "Quarterly Returns, Combinations: (5, 2)")
    
    # calculating KPIs for 5,2 monthly rebalancing
    portfolio_quarterly_KPI = KPI(portfolio_returns_quarterly,market_returns_quarterly_testing,"3mo")
